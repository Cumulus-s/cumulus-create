...
Hosted Relay uses external payment provider for checkout, subscriptions, invoices, and credit
packs. Self-hosted operators can keep external payment provider, replace it, or turn enforcement
off while testing.
...
2. Connect external payment provider and verify webhook handling in test mode.
