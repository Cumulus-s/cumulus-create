...
- **Rust pillar:** `cmls-knowledge-core` and `cmls-knowledge-cli`. This is the trusted local engine. It owns indexing, SQLite storage, semantic graph construction, CLI, TUI, local MCP, and local API serving.
- **TypeScript pillar:** `@cmls/knowledge`. This owns the no-code API shape, browser graph UI, Anime.js interactions, MCP Apps/web surfaces, and the main human-facing SDK.
- **Python pillar:** `cmls-knowledge`. This owns operations ingestion helpers, batch enrichment, evals, and AI-agent analysis workflows.
