...
function delegateKnowledge(knowledgeArgs: string[]) {
  delegateCommand(env.CUMULUS_KNOWLEDGE_BIN || "cmls-knowledge", [
    "knowledge",
    ...knowledgeArgs,
  ]);
}
...
function delegateCreateCumulus(commandArgs: string[]) {
  try {
    delegate(execPath, [require.resolve("@cmls/create"), ...commandArgs]);
  } catch {
...
async function status() {
  ...
  const knowledge = spawnSync(env.CUMULUS_KNOWLEDGE_BIN || "cmls-knowledge", ["--version"], {
    encoding: "utf8",
  });
  checks.knowledge = {
