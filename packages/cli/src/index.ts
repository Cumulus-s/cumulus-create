#!/usr/bin/env node
/**
 * @cmls/cli — first-class command surface for Relay.
 *
 ...
 *
 *   relay init        (scaffold @cmls/auth webhook into a Next.js project)
 *
...
interface RelayConfig {
  ...
  /**
   * Active tenant for tenant-scoped commands (`products`, `stats`,
...
// ---------------------------------------------------------------------------
// Commands: local plan status while Cumulus billing is redesigned.
// ---------------------------------------------------------------------------
...
async function cmdSubscription(args: ParsedArgs): Promise<void> {
  ...
  console.log(bold('Tenant plan status') + dim(`  (${tenantId})`));
  console.log(dim('  Self-serve payment flows are disabled until Cumulus billing is redesigned.'));
...
async function cmdSubscribe(args: ParsedArgs): Promise<void> {
  const cfg = await requireConfig();
  resolveTenantId(cfg, args);
  const body = {
    error: 'payments_disabled',
    message: 'Self-serve payment flows are disabled until Cumulus billing is redesigned.',
  };
  if (args.flags.json) {
...
${bold('Usage')}
  subscription [--tenant id]  Local tenant plan snapshot
  subscribe [--plan builder]  Disabled until Cumulus billing is redesigned
