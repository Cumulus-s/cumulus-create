...
for (const result of results) {
  if (result.skipped) process.stderr.write(`[cumulus-knowledge] skipped ${result.runtime}: ${result.reason}\n`);
}
if (failed.length > 0) {
  for (const result of failed) process.stderr.write(`[cumulus-knowledge] failed ${result.runtime}: ${result.reason}\n`);
  process.exit(1);
}
...
function printHelp(): void {
  process.stdout.write(`Usage: cumulus-knowledge-setup [--all|--rust|--python] [options]
