-- Agent-driven payments: external payment provider Shared Payment Tokens (SPT), Machine Payments
...
-- autopay endpoint. external payment provider enforces the per-transaction cap + expiration;
...
--    and MPP per-call charges. Separate from usage_events so the external payment provider-side
...
-- 3) user_issued_cards: schema-only scaffolding for external payment provider Issuing.
...
CREATE TABLE IF NOT EXISTS "user_issued_cards" (
  "id"                            uuid PRIMARY KEY DEFAULT gen_random_uuid() NOT NULL,
  "user_id"                       uuid NOT NULL,
  "external_cardholder_id"          text NOT NULL,
