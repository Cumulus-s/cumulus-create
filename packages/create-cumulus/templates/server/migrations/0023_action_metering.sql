...
--   * usage_pending_invoice_items.idempotency_key — generic key (was
...
-- external payment provider webhook (`resetQuotaForPeriod`) reads the new
...
-- (external payment provider Price IDs unchanged) but inflate quotas and re-anchor overage
...
-- without touching their external payment provider subscription. Charge middleware reads
...
ALTER TABLE "usage_pending_invoice_items"
  ADD COLUMN IF NOT EXISTS "idempotency_key" text;--> statement-breakpoint

UPDATE "usage_pending_invoice_items"
...
CREATE UNIQUE INDEX IF NOT EXISTS "usage_pending_invoice_items_idempotency_key_key"
  ON "usage_pending_invoice_items" ("idempotency_key");--> statement-breakpoint
