...
export const tenants = pgTable('tenants', {
  ...
  // contain runaway abuse without touching the external payment provider subscription.
  ...
  // Operator-driven; not exposed to public catalog. The external payment provider webhook
...
// ---------------------------------------------------------------------------
// tenant_subscriptions — external payment provider-backed tenant subscription state. A tenant
...
// backfill script; real plans are managed by external payment provider webhooks.
...
export const tenant_subscriptions = pgTable('tenant_subscriptions', {
  ...
  external_subscription_id: text('external_subscription_id').unique(),
  external_customer_id: text('external_customer_id'),
  ...
  //                      the external payment provider webhook on subscription.updated)
  ...
  // Sourced from external payment provider webhooks (price.recurring.interval) — request
...
// `external_event_id` is the external payment provider webhook event id and doubles as our
// idempotency key (UNIQUE). `event_type` mirrors external payment provider ('created',
// 'renewed', 'canceled', 'past_due', …). 'created' rows with
// external_event_id NULL come from the initial backfill.
// ---------------------------------------------------------------------------
...
// meter. Editable in place; the external payment provider webhook upsert re-reads on every
...
// ---------------------------------------------------------------------------
// usage_pending_invoice_items — queue of per-action overage charges that
// haven't been pushed to external payment provider yet. Flushed monthly by the cron at
...
// ---------------------------------------------------------------------------
export const usage_pending_invoice_items = pgTable(
  'usage_pending_invoice_items',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    tenant_id: uuid('tenant_id')
      .references(() => tenants.id, { onDelete: 'cascade' })
...
//
// An integrator buys a one-shot credit pack via external payment provider Checkout
...
export const action_credits = pgTable('action_credits', {
  ...
  external_payment_intent_id: text('external_payment_intent_id').unique(),
  external_checkout_session_id: text('external_checkout_session_id'),
  expires_at: timestamp('expires_at', { withTimezone: true }).notNull(),
  created_at: timestamp('created_at', { withTimezone: true }).notNull().defaultNow(),
  updated_at: timestamp('updated_at', { withTimezone: true }).notNull().defaultNow(),
});
...
// 429 refusal past the cap. idempotency_key is client-supplied (external payment provider
...
// `secret_value` at rest (external payment provider restricted-key pattern) and uses
