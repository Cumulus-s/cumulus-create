...
describe('parseCliArgs', () => {
  ...
  it('parses centralized technology and runtime flags', () => {
    expect(
      parseCliArgs([
        'demo',
        '--with',
      ...
      technologies: ['auth', 'db', 'knowledge'],
  ...
  it('rejects unknown technologies before writing files', () => {
    expect(() => parseCliArgs(['demo', '--with', 'auth,unknown'])).toThrow(
      /invalid --with/,
    );
  });
...
describe('buildFiles', () => {
  it('renders all template/auth combinations', () => {
    for (const template of templateNames) {
      for (const agentAuth of agentAuthModes) {
        ...
        expect(files.get('package.json')).toContain('@cmls/auth');
        ...
        if (template === 'marketing') {
          ...
          expect(files.get('package.json')).toContain('cumulus-db:nimbus:check');
          expect(files.get('package.json')).toContain('"pg": "^8.21.0"');
          expect(files.get('apps/cumulus-db/src/config.ts')).toContain('CumulusDbConfigEnv');
          expect(files.has('apps/cumulus-db/src/nimbus.ts')).toBe(true);
          expect(files.has('apps/cumulus-db/src/system.ts')).toBe(true);
          expect(files.has('apps/cumulus-db/openapi/system-v1.openapi.json')).toBe(true);
          expect(files.has('apps/cumulus-db/schemas/nimbus-ir-v1alpha1.schema.json')).toBe(true);
          expect(files.has('apps/cumulus-db/postgres/data-v1.sql')).toBe(true);
        }
  ...
  it('keeps cloud-only agent auth MIT and excludes local Cumulus DB files', () => {
    ...
    expect(files.get('package.json')).toContain('"@cmls/db"');
    expect(files.get('package.json')).not.toContain('"@cmls/nimbus"');
    expect(files.has('apps/cumulus-db/package.json')).toBe(false);
  ...
  it('adds local Cumulus DB service files, scripts, env, and AGPL license', () => {
    ...
    expect(packageJson).toContain('"@cmls/nimbus": "^0.1.0"');
    ...
    expect(packageJson).toContain('"pg": "^8.21.0"');
    expect(files.get('.env.example')).toContain('CUMULUS_DB_MASTER_KEY');
    expect(files.get('.env.example')).toContain('CUMULUS_DB_DATA_DIR');
    expect(files.has('apps/cumulus-db/src/nimbus-cli.ts')).toBe(true);
    expect(files.has('apps/cumulus-db/src/postgres-engine.ts')).toBe(true);
    expect(files.get('README.md')).toContain('Relay Postgres');
    expect(files.get('README.md')).toContain('Nimbus schema tooling');
