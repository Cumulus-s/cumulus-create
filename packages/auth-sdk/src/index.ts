 ...
 * Cumulus Auth sends one of these to the integrator.
 */
...
export type CumulusWebhookPayload =
  | SignupPayload
  | CreateApiKeyPayload
  | RevokeApiKeyPayload
  | TeardownPayload;

/** Backward-compatible type alias for older Relay integrations. */
export type RelayWebhookPayload = CumulusWebhookPayload;
...
export interface WebhookOptions {
  /** HMAC-SHA256 shared secret. MUST match the one configured in Cumulus Auth. */
  secret: string;
  ...
  /** Optional local event ledger hook. Emits safe metadata only. */
  ...
  const handler = async (req: Request): Promise<Response> => {
    ...
    if (!(await verifySignature(rawBody, sig, options.secret))) {
      await events?.emit({
        ...
        metadata: { error_class: 'invalid_signature' },
      });
    ...
    try {
      ...
      await events?.emit({
        ...
        durationMs: Date.now() - started,
        privacy: { class: 'creator_safe_metadata', redacted: false },
        metadata: { error_class: 'invalid_json' },
      });
      return jsonResponse(400, { error: 'invalid_json' });
    }

    const p = payload as Partial<CumulusWebhookPayload>;
    try {
      switch (p.kind) {
        case 'signup': {
          if (
            typeof p.email !== 'string' ||
            typeof p.signupId !== 'string' ||
            typeof p.provider_slug !== 'string'
          ) {
          ...
          await events?.emit({
            ...
            durationMs: Date.now() - started,
            refs: {
              accountRef: hashString(result.externalId ?? result.accountId),
              credentialRef: hashString(`${p.signupId}:initial-key`),
            },
            privacy: { class: 'creator_safe_metadata', redacted: false },
            metadata: {
              provider_slug: p.provider_slug,
              signup_ref: hashString(p.signupId),
              input_field_count: Object.keys(p.input ?? {}).length,
            },
          });
        ...
        case 'create_api_key': {
          ...
          await events?.emit({
            ...
            durationMs: Date.now() - started,
            refs: {
              accountRef: hashString(p.account_id),
              credentialRef: result.providerKeyId ? hashString(result.providerKeyId) : undefined,
            },
            privacy: { class: 'creator_safe_metadata', redacted: false },
            metadata: { label_hash: hashString(p.label) },
          });
          return jsonResponse(200, result);
        }
        case 'revoke_api_key': {
          ...
          await events?.emit({
            ...
            refs: {
              accountRef: hashString(p.account_id),
              credentialRef: hashString(p.key_id),
            },
            privacy: { class: 'creator_safe_metadata', redacted: false },
          });
        ...
        case 'teardown': {
          ...
          await events?.emit({
            ...
            status: 'ok',
            durationMs: Date.now() - started,
            refs: { accountRef: hashString(p.account_id) },
            privacy: { class: 'creator_safe_metadata', redacted: false },
          });
      ...
      await events?.emit({
        operation: typeof p.kind === 'string' ? p.kind : 'webhook',
        status: 'error',
        durationMs: Date.now() - started,
        privacy: { class: 'creator_safe_metadata', redacted: false },
        metadata: { error_class: err instanceof Error ? err.name : 'Error' },
      });
...
/** Payload Cumulus Auth POSTs to your registered `endpoint_url` on every invocation. */
 ...
 * 500. Cumulus Auth folds the `output` back into the response to the agent.
 *
...
// ---------------------------------------------------------------------------
// Outbound client — server-side calls to Cumulus Auth that don't fit the webhook-
...
export interface ClientOptions {
  ...
  agentToken: string;
  /** Defaults to the hosted Cumulus Auth API. */
  baseUrl?: string;
...
async function clientRequest<T>(
  ...
  const base = opts.baseUrl ?? 'https://auth.cumulus.sh';
...
/** Backward-compatible alias for older Relay integrations. */
export const relay = cumulus;
