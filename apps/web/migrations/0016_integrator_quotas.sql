...
-- as usage-based overage via a per-signup external payment provider invoice item, flushed
...
-- subscription event, so price changes land on the next external payment provider billing
...
-- --------------------------------------------------------------------------
-- usage_pending_invoice_items — queue of per-signup overage charges that
...
-- --------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS "usage_pending_invoice_items" (
...
CREATE INDEX IF NOT EXISTS "usage_pending_invoice_items_tenant_flushed_idx"
  ON "usage_pending_invoice_items" ("tenant_id", "flushed_at");
