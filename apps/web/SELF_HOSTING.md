...
- Optional external payment provider account for billing
...
| Variable | Purpose |
| --- | --- |
| `BILLING_ENFORCEMENT` | Local quota gate mode. Use `off`, `warn`, or `enforce`. |
| `BILLING_FAIRNESS` | Repeat-user fairness switch for key lifecycle actions. |
