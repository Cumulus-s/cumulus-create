...
import { listCredits } from '../server/billing/credits';
import {
...
export function buildMcpServer(): McpServer {
  ...
  //     Returns the current tenant's local plan and quota snapshot. Callable
  ...
  server.registerTool(
    ...
    {
      description:
        "Return the named tenant's local plan and quota snapshot. Caller must own or be a member of the tenant. Read-only; no external payment action is started.",
      inputSchema: {
        agent_token: z.string(),
        tenant_id: z
          .string()
  ...
  // -------------------------------------------------------------------------
  // 15. whoami — public (accepts optional agent_token).
