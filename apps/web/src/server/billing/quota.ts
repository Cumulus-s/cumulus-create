 ...
 *      the helper queues a row in usage_pending_invoice_items (overage)
...
/** Drop the plan cache — call from the external payment provider webhook on plan_catalog edits. */
...
async function latestSubscription(tenantId: string): Promise<{
  ...
  const [row] = await db
    .select({
      ...
      external_subscription_id: tenant_subscriptions.external_subscription_id,
    })
...
export interface QuotaClaim {
   ...
   *   - 'overage'  → queued a usage_pending_invoice_items row
   */
...
export async function requireIntegratorQuota(args: ClaimArgs): Promise<QuotaClaim | null> {
  ...
  await db
    .insert(usage_pending_invoice_items)
    .values({
      tenant_id: args.tenantId,
      signup_job_id: args.signupJobId ?? null,
      idempotency_key: key,
      amount_cents: plan.overage_price_cents,
      external_subscription_id: sub?.external_subscription_id ?? null,
    })
    .onConflictDoNothing({ target: usage_pending_invoice_items.idempotency_key });
...
export async function refundIntegratorQuota(args: ClaimArgs): Promise<void> {
  ...
  // 1. Was this claim on the overage queue and not yet flushed to external payment provider?
  const deleted = await db
    .delete(usage_pending_invoice_items)
    .where(
      and(
        eq(usage_pending_invoice_items.tenant_id, args.tenantId),
        eq(usage_pending_invoice_items.idempotency_key, key),
      ),
    )
    .returning({ id: usage_pending_invoice_items.id });
...
/**
 * Called from the external payment provider webhook when a billing period starts or a plan
