...
//   2. No live external payment provider subscription exists (status in trialing|active|past_due).
...
// usage_pending_invoice_items, scale_benchmark_samples) drops with the tenant.
...
app.openapi(
  createRoute({
    ...
    description:
      ...
      'Requires the caller to be the owner, no live external payment provider subscription, and ' +
    ...
    responses: {
      ...
      409: {
        description:
          'A live external payment provider subscription exists — cancel via /dev/billing first.',
        content: {
          'application/json': {
            schema: z.object({
